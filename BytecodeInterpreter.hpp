...
class InstrumentBCInterpreter{
	public: 
		static std::mutex myMutex;
		uint64_t staticCounter = 0;
		uint64_t virtualCounter = 0;
		void incrementStatic() {
			staticCounter++;
		}
		void incrementVirtual() {
			virtualCounter++;
		}
		~InstrumentBCInterpreter() {
			static bool toPrint = getenv("ENABLE_INSTR") ? true : false;
			if (toPrint) {
				std::lock_guard<std::mutex> lock(myMutex);
				std::cerr << staticCounter << "," << virtualCounter << std::endl;
			}
		}
};
...
private:
	...
	InstrumentBCInterpreter callSiteCounter;
...
retry:
	...
	{
		callSiteCounter.incrementVirtual();
	...
	{
		callSiteCounter.incrementStatic();
		U_16 index = *(U_16*)(_pc + 1);
		profileCallingMethod(REGISTER_ARGS);
